...
# Generate key baseline
flutter_keycheck baseline create --out baseline.yaml
```
...
| AST Scanner | Parse Dart files for keys | Duplicated (v2/v3) |
| Key Detectors | Pattern matching | Working |
| Cache Manager | Performance optimization | Incomplete |
| Reporters | Output generation | Triple implementation |
...
|--------|-------------|---------|
| --scope | Package scanning scope | workspace-only, deps-only, all |
| --report | Output format | html, json, md, junit |
| --baseline | Baseline file | keys.yaml |
| --config | Config file | .flutter_keycheck.yaml |
