...
class PlayingCard {
  ...
  bool get isFaceCard =>
      [CardRank.jack, CardRank.queen, CardRank.king].contains(rank);
...
class BlackjackHand {
  ...
  bool get canSplit {
    return _cards.length == 2 && _cards[0].rank == _cards[1].rank;
  }
...
class BlackjackGameResult {
  ...
  bool get hasBlackjack =>
      playerHands.any((h) => h.result == BlackjackResult.blackjack);
}
...
class BlackjackEngine {
  ...
  bool startNewGame(int bet) {
    ...
    if (_playerHands[0].isBlackjack) {
      if (_dealerHand.cards[0].rank.blackjackValue == 10 ||
          _dealerHand.cards[0].isAce) {
  ...
  List<BlackjackAction> getAvailableActions() {
    ...
    if (_canPerformAction(BlackjackAction.hit))
      actions.add(BlackjackAction.hit);
    if (_canPerformAction(BlackjackAction.stand))
      actions.add(BlackjackAction.stand);
    if (_canPerformAction(BlackjackAction.doubleDown))
      actions.add(BlackjackAction.doubleDown);
    if (_canPerformAction(BlackjackAction.split))
      actions.add(BlackjackAction.split);
    if (_canPerformAction(BlackjackAction.surrender))
  ...
  BlackjackAction getBasicStrategyAction() {
    ...
    final canDouble =
        getAvailableActions().contains(BlackjackAction.doubleDown);

    // Basic strategy logic (simplified)
    if (currentHand.canSplit &&
        getAvailableActions().contains(BlackjackAction.split)) {
      ...
      if (pairValue == 10 || pairValue == 5 || pairValue == 4) {
        ...
        if ((pairValue == 2 || pairValue == 3) && dealerUpCard <= 7)
        ...
        if (pairValue == 9 &&
            dealerUpCard != 7 &&
            dealerUpCard != 10 &&
            dealerUpCard != 11) return BlackjackAction.split;
      }
    }
    ...
    if (playerValue == 9 && canDouble && dealerUpCard >= 3 && dealerUpCard <= 6)
      return BlackjackAction.doubleDown;
    if (playerValue == 10 && canDouble && dealerUpCard <= 9)
      return BlackjackAction.doubleDown;
    if (playerValue == 11 && canDouble && dealerUpCard <= 10)
      return BlackjackAction.doubleDown;
  ...
  void _dealInitialCards() {
    ...
    if (dealerCard2 != null)
      _dealerHand.addCard(dealerCard2.copyWith(isHidden: true));
  }
