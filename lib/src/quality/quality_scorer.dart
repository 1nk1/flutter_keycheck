...
class QualityScorer {
  ...
  static double _calculateConsistencyScore(
    ...
    if (keyUsageCounts != null) {
      final multiUseKeys =
          keyUsageCounts.entries.where((entry) => entry.value > 1).length;
  ...
  static double _calculateEfficiencyScore(
    ...
    if (extraKeys.isNotEmpty) {
      final extraRatio =
          extraKeys.length / (foundKeys.length + extraKeys.length);
      if (extraRatio > 0.3) {
        score -= 30.0;
    ...
    if (keyUsageCounts != null && expectedKeys.isNotEmpty) {
      ...
      final averageUsage = expectedFound.isEmpty
          ? 0.0
          : expectedFound
                  .map((key) => keyUsageCounts[key] ?? 0)
                  .reduce((a, b) => a + b) /
              expectedFound.length;

      if (averageUsage > 2.0) {
        score += 15.0; // Keys are well used
      } else if (averageUsage > 1.5)
        score += 10.0;
  ...
  static double _calculateMaintainabilityScore(
    ...
    final clearNames = foundKeys
        .where((key) => key.length > 3 && key.contains(RegExp(r'[A-Z]|_')))
        .length;
  ...
  static List<String> _generateRecommendations(
    ...
    if (coverage < 80.0) {
      recommendations.add(
          '🎯 Add missing keys to improve test coverage (${missingKeys.length} missing)');
    }
    if (missingKeys.length > 5) {
      recommendations
          .add('⚡ Focus on critical missing keys first for immediate impact');
    }

    // Organization recommendations
    if (organization < 70.0) {
      recommendations
          .add('📁 Improve key organization by grouping related keys together');
      recommendations
          .add('🏗️ Consider using KeyConstants class for better organization');
    }

    // Consistency recommendations
    if (consistency < 70.0) {
      recommendations
          .add('🔄 Establish consistent naming patterns for all keys');
      if (keyUsageCounts != null) {
        final duplicates =
            keyUsageCounts.entries.where((e) => e.value > 1).length;
        if (duplicates > 0) {
          recommendations.add(
              '🔍 Review $duplicates duplicate keys for potential consolidation');
        }
      }
    }
    ...
    if (efficiency < 70.0) {
      if (extraKeys.isNotEmpty) {
        recommendations.add(
            '🧹 Remove ${extraKeys.length} unused/extra keys to reduce noise');
      }
      recommendations.add(
          '⚡ Optimize key usage patterns for better automation efficiency');
    }
    ...
    if (recommendations.isEmpty) {
      recommendations.add(
          '✨ Excellent key organization! Continue monitoring for consistency');
    }
  ...
  static Map<String, dynamic> _calculateDetailedMetrics(
    ...
    metrics['coveragePercentage'] = expectedKeys.isEmpty
        ? 100.0
    ...
    if (keyUsageCounts != null) {
      ...
      if (usageCounts.isNotEmpty) {
        metrics['averageUsage'] =
            usageCounts.reduce((a, b) => a + b) / usageCounts.length;
        metrics['maxUsage'] = usageCounts.reduce(math.max);
        metrics['duplicateKeys'] =
            usageCounts.where((count) => count > 1).length;
      }
    }
    ...
    if (keyLocations != null && scannedFiles != null) {
      ...
      metrics['keyDistribution'] = scannedFiles.isEmpty
          ? 0.0
  ...
  static bool _checkNamingConsistency(Set<String> keys) {
    ...
    final camelCase =
        keys.where((k) => k.contains(RegExp(r'[a-z][A-Z]'))).length;
  ...
  static double _calculateNamingPatternScore(Set<String> keys) {
    ...
    final semantic = keys
        .where((k) => k
            .toLowerCase()
            .contains(RegExp(r'button|field|text|icon|screen|form|menu|item')))
        .length;
  ...
  static Map<String, dynamic> _analyzeNamingPatterns(Set<String> keys) {
    ...
    final semantic = keys
        .where((k) => k.toLowerCase().contains(RegExp(
            r'button|field|text|icon|screen|form|menu|item|widget|dialog')))
        .length;
    ...
    return {
      ...
      'consistencyPattern':
          _getDominantPattern(camelCase, snakeCase, kebabCase),
    };
  }

  /// Get the dominant naming pattern
  static String _getDominantPattern(
      int camelCase, int snakeCase, int kebabCase) {
