...
class CoverageReporter {
  ...
  String _generateJsonReport(bool includeDetails, bool includeEvidence) {
    final report = <String, dynamic>{
      ...
      // Blind spots
      'blind_spots': scanResult.blindSpots.map((bs) => {
        'type': bs.type,
      ...
      // Errors
      'errors': scanResult.metrics.errors.map((e) => {
        'file': e.file,
        'type': e.type,
        'error': e.error,
      }).toList(),
...
''');
    ...
    _addMetricCard(buffer, 'File Coverage', 
        scanResult.metrics.fileCoverage, '%');
    _addMetricCard(buffer, 'Widget Coverage', 
        scanResult.metrics.widgetCoverage, '%');
    _addMetricCard(buffer, 'Handler Coverage', 
        scanResult.metrics.handlerCoverage, '%');
    _addMetricCard(buffer, 'Total Keys', 
        scanResult.keyUsages.length.toDouble(), '');
    ...
    buffer.writeln('<tr><th>Detector</th><th>Hits</th><th>Keys Found</th><th>Effectiveness</th></tr>');
    
    for (final entry in scanResult.metrics.detectorHits.entries) {
      final effectiveness = _getDetectorEffectiveness(entry.key);
      final cssClass = effectiveness > 75 ? 'good' : 
                       effectiveness > 25 ? 'warning' : 'error';
      buffer.writeln('''
        <tr>
          <td>${entry.key}</td>
          <td>${entry.value}</td>
    ...
    if (includeDetails) {
      ...
      buffer.writeln('<table>');
      buffer.writeln('<tr><th>File</th><th>Keys</th><th>Widgets</th><th>Coverage</th><th>Actions</th></tr>');
      ...
      for (final entry in scanResult.keyUsages.entries.take(20)) {
        ...
        if (entry.value.handlers.isNotEmpty) {
          ...
          for (final handler in entry.value.handlers) {
            buffer.writeln('${handler.type}: ${handler.method ?? "anonymous"}<br>');
          }
        }
  ...
  String _generateMarkdownReport(bool includeDetails, bool includeEvidence) {
    ...
    buffer.writeln('|--------|-------|---------|----------|');
    buffer.writeln('| Files | ${scanResult.metrics.totalFiles} | ${scanResult.metrics.scannedFiles} | ${scanResult.metrics.fileCoverage.toStringAsFixed(1)}% |');
    buffer.writeln('| Widgets | ${_getTotalWidgets()} | ${_getWidgetsWithKeys()} | ${scanResult.metrics.widgetCoverage.toStringAsFixed(1)}% |');
    buffer.writeln('| Handlers | ${_getTotalHandlers()} | ${_getLinkedHandlers()} | ${scanResult.metrics.handlerCoverage.toStringAsFixed(1)}% |');
    ...
    for (final entry in scanResult.metrics.detectorHits.entries) {
      ...
      final indicator = effectiveness > 75 ? '✅' : 
                        effectiveness > 25 ? '⚠️' : '❌';
      buffer.writeln('| ${entry.key} | ${entry.value} | ${_getDetectorKeyCount(entry.key)} | $indicator ${effectiveness.toStringAsFixed(1)}% |');
    }
    ...
    if (includeDetails) {
      ...
      for (final entry in sortedFiles.take(10)) {
        ...
        buffer.writeln('| `${analysis.relativePath}` | ${analysis.keysFound.length} | ${analysis.widgetCount} | ${coverage.toStringAsFixed(1)}% |');
      }
    }
    ...
    if (scanResult.blindSpots.isNotEmpty) {
      ...
      for (final blindSpot in scanResult.blindSpots) {
        final icon = blindSpot.severity == 'error' ? '🔴' :
                     blindSpot.severity == 'warning' ? '🟡' : 'ℹ️';
        buffer.writeln('$icon **${blindSpot.type}** at `${blindSpot.location}`');
    ...
    if (includeEvidence) {
      ...
      for (final entry in linkedKeys) {
        ...
        for (final loc in entry.value.locations.take(3)) {
          buffer.writeln('- `${path.relative(loc.file, from: projectPath)}:${loc.line}` (${loc.detector})');
        }
        
        buffer.writeln('\n**Handlers:**');
        for (final handler in entry.value.handlers) {
          buffer.writeln('- ${handler.type}: `${handler.method ?? "anonymous"}`');
        }
        buffer.writeln('');
      }
    }
    ...
    if (violations.isNotEmpty) {
      ...
      for (final violation in violations) {
        buffer.writeln('- **${violation['metric']}**: ${violation['actual']}% (threshold: ${violation['threshold']}%)');
      }
    }
  ...
  String _generateJUnitReport() {
    ...
    buffer.writeln('<testsuites name="Flutter KeyCheck Coverage" tests="$totalTests" failures="$failures">');
    buffer.writeln('  <testsuite name="Coverage" tests="$totalTests" failures="$failures">');
    
    // File coverage test
    _addJUnitTest(buffer, 'File Coverage', 
        scanResult.metrics.fileCoverage,
        thresholds['file_coverage'] as double? ?? 80.0);
    
    // Widget coverage test
    _addJUnitTest(buffer, 'Widget Coverage',
        scanResult.metrics.widgetCoverage,
        thresholds['widget_coverage'] as double? ?? 70.0);
    
    // Handler coverage test
    _addJUnitTest(buffer, 'Handler Coverage',
        scanResult.metrics.handlerCoverage,
        thresholds['handler_coverage'] as double? ?? 60.0);
    
    // Blind spots test
    final hasBlindSpots = scanResult.blindSpots
        .any((bs) => bs.severity == 'error');
    
    buffer.writeln('    <testcase name="No Critical Blind Spots" classname="Coverage">');
    if (hasBlindSpots) {
      buffer.writeln('      <failure message="Critical blind spots detected">');
      for (final bs in scanResult.blindSpots.where((b) => b.severity == 'error')) {
  ...
  String _generateLcovReport() {
    ...
    for (final entry in scanResult.fileAnalyses.entries) {
      ...
      buffer.writeln('FNF:${analysis.functions.length + analysis.methods.length}');
      buffer.writeln('FNH:${analysis.keysFound.length}'); // Keys as "hit" functions
  ...
  String _generateTextReport(bool includeDetails, bool includeEvidence) {
    ...
    if (scanResult.metrics.incrementalScan) {
      buffer.writeln('Mode: Incremental (base: ${scanResult.metrics.incrementalBase})');
    } else {
    ...
    buffer.writeln('────────────────');
    _addTextMetric(buffer, 'File Coverage', 
        scanResult.metrics.scannedFiles, 
        scanResult.metrics.totalFiles,
        scanResult.metrics.fileCoverage);
    _addTextMetric(buffer, 'Widget Coverage',
        _getWidgetsWithKeys(),
        _getTotalWidgets(),
        scanResult.metrics.widgetCoverage);
    _addTextMetric(buffer, 'Handler Coverage',
        _getLinkedHandlers(),
        _getTotalHandlers(),
        scanResult.metrics.handlerCoverage);
    ...
    for (final entry in scanResult.metrics.detectorHits.entries) {
      ...
      final status = effectiveness > 75 ? '✓' : 
                     effectiveness > 25 ? '!' : '✗';
      buffer.writeln('[$status] ${entry.key.padRight(20)} ${entry.value.toString().padLeft(6)} hits  ${effectiveness.toStringAsFixed(1).padLeft(5)}% effective');
    }
    ...
    if (scanResult.blindSpots.isNotEmpty) {
      ...
      for (final blindSpot in scanResult.blindSpots) {
        final icon = blindSpot.severity == 'error' ? '[ERROR]' :
                     blindSpot.severity == 'warning' ? '[WARN] ' : '[INFO] ';
    ...
    if (violations.isNotEmpty) {
      ...
      for (final violation in violations) {
        buffer.writeln('[FAIL] ${violation['metric']}: ${violation['actual']}% < ${violation['threshold']}%');
      }
    }
    ...
    if (includeDetails) {
      ...
      for (final entry in sortedFiles.take(10)) {
        ...
        buffer.writeln('   Widgets: ${analysis.widgetCount} (${analysis.widgetsWithKeys} with keys)');
      ...
      if (includeEvidence) {
        ...
        for (final usage in scanResult.keyUsages.entries) {
          ...
          if (locations.isNotEmpty) {
            fileKeys[usage.key] = locations.map((loc) => {
              'line': loc.line,
              'column': loc.column,
  ...
  Map<String, dynamic> _getEvidence() {
    ...
    for (final entry in scanResult.keyUsages.entries) {
      evidence[entry.key] = {
        'locations': entry.value.locations.map((loc) => {
        ...
        'handlers': entry.value.handlers.map((h) => {
          'type': h.type,
          'method': h.method,
  ...
  List<Map<String, dynamic>> _checkThresholds() {
    ...
    // Check file coverage
    final fileCoverageThreshold = thresholds['file_coverage'] as double? ?? 80.0;
    ...
    final widgetCoverageThreshold = thresholds['widget_coverage'] as double? ?? 70.0;
    ...
    final handlerCoverageThreshold = thresholds['handler_coverage'] as double? ?? 60.0;
  ...
  void _addMetricCard(StringBuffer buffer, String label, double value, String unit) {
  ...
  void _addJUnitTest(StringBuffer buffer, String name, double actual, double threshold) {
  ...
  void _addTextMetric(StringBuffer buffer, String label, int covered, int total, double percentage) {
    final bar = _generateProgressBar(percentage);
    buffer.writeln('${label.padRight(20)} $bar ${percentage.toStringAsFixed(1).padLeft(5)}% ($covered/$total)');
  }
